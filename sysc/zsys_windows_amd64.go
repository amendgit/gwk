// mksyscall_windows.pl sys_windows.go sys_windows_amd64.go
...
func GetWindowLongPtr(hwnd Handle, index int64) (longptr uintptr) {
	r0, _, _ := syscall.Syscall(procGetWindowLongPtrW.Addr(), 2, uintptr(hwnd), uintptr(index), 0)
	longptr = uintptr(r0)
	return
}

func SetWindowLongPtr(hwnd Handle, index int64, longptr uintptr) (oldLong uintptr) {
	r0, _, _ := syscall.Syscall(procSetWindowLongPtrW.Addr(), 3, uintptr(hwnd), uintptr(index), uintptr(longptr))
	oldLong = uintptr(r0)
	return
}
